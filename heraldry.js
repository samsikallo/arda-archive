...
window.ardaChargePath=function(kind,cx,cy,r,fill,ac){
 ...
 switch(kind){
  ...
  case"tengwa":var ser=(window.ARDA_TENGWA_SER||4),grd=(window.ARDA_TENGWA_GRADE||2);
    var closed=(ser===2||ser===4),dbl=(grd===2||grd===4),raised=(grd===3||grd===4),red=(grd>=5);
    var sw=Math.max(1.8,r*0.11),sx=cx-r*0.50;
    var by=cy-r*0.05,bh=r*0.66,bw=r*0.60;                 // the bow: its top, half-height, width
    var top=raised?cy-r*1.20:(red?by-bh:by-bh),bot=red?by+bh:cy+r*1.20;
    var tw='<path d="M'+F(sx)+','+F(top)+' V'+F(bot)+'" stroke="'+fill+'" stroke-width="'+F(sw)+'" stroke-linecap="round" fill="none"/>';
    function luva(x0){
      var d='M'+F(x0)+','+F(by-bh)+' C'+F(x0+bw*1.6)+','+F(by-bh*0.86)+' '+F(x0+bw*1.6)+','+F(by+bh*0.86)+' '+F(x0)+','+F(by+bh);
      if(closed)return '<path d="'+d+'Z" fill="'+fill+'"/>';
      return '<path d="'+d+'" fill="none" stroke="'+fill+'" stroke-width="'+F(sw)+'" stroke-linecap="round"/>'}
    tw+=luva(sx); if(dbl)tw+=luva(sx+bw*0.92);
    return tw;
  // "their emblems the moon and the sun and the scarlet heart" (FoG) -- all three of the
  // King's house, set one above another as they stand on the plate.
